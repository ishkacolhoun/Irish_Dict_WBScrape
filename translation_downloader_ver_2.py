...
for word in words_list:
    ...
    translation = translation.replace(' f.', ' feminine ').replace(' poss.', ' possesive ').replace(' a.', ' adjective ').replace(' sg.', ' singular ').replace(' pl.', ' plural ').replace(' vn.', ' verbal noun ').replace('(vn.', '(verbal noun ')
    translation = translation.replace(' gs.', ' genetive singular ').replace(' var.', ' variaton ').replace(' Lit:', ' Literatry use: ').replace(' s.', ' substantive ').replace(' int.', ' interjection ').replace(' npl.', ' nominitave plural ').replace(' a.', ' adjective ').replace('(gsm.', '(genetive singular masculine ').replace('(gsf.', '(genetive singular feminine ')
    translation = translation.replace(' comp.', ' comparitive ').replace(' npl.', ' nominative plural ').replace(' Prov:', ' Proverb: ').replace(' v.t.', ' transitive verb ').replace(' pp.', ' past particle ').replace(' f,', ' feminine, ').replace(' m,', ' masculine, ').replace('gpl.', ' genetive plural ')
    ...
    #     output_file.writerow(this_dict.keys())
    #     output_file.writerow(this_dict.values())
    present_word_index = words_list.index(word)
    total_word_number = len(words_list)
    print('Percentage Completed:',round(present_word_index/total_word_number * 100,4),'%')
    #percentage_done = 
    output_file1.flush()
output_file1.close()
